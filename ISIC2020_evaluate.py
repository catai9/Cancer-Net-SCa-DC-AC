...
from helpers.DarwinNet_659MF import DarwinNetV2
from helpers.ISIC2020_Dataset import ISIC2020TorchDataset, NORM_CHANNEL_MEAN, NORM_CHANNEL_STD


parser = argparse.ArgumentParser(description='CancerNet-SCa x AttendNeXt Inferece')
parser.add_argument('--weightspath', default='models/Cancer-Net SCa-DC-AC.pth', type=str, help='Path to trained model weights')
parser.add_argument('-mc', '--model-config', default='config/DarwinNet_XA_325MF.yml', help='Path to model config file')
parser.add_argument('-dd', '--data-dir', type=str, default='sample-data/images/', help='Sample image directory')
parser.add_argument('-tf', '--split-file', type=str, default='sample-data/sample_data_results.csv', help='CSV file with actual results for image directory')
